...
- `P1` 압축파일 안 검색(파일명 필터) — ✅ 완료 (현재 폴더 안에서만, 하위 폴더
  재귀 검색은 아님)
- `P2` 중첩 압축(zip 안의 zip 등) 드릴다운 — ✅ 완료. 항목 이름으로 압축
  형식이 인식되면 미리보기 대신 그 안으로 곧장 들어간다(재귀적으로 몇
  단계든 가능 — `ArchiveBrowserScreen`이 자기 자신을 다시 push). 지금
  리더가 못 읽는 형식(예: iso9660)이면 평소 미리보기로 조용히 대체된다.
  알려진 한계: `archive` 패키지의 zip/tar 디코더가 매직 바이트 없는
  데이터도 "항목 0개"로 관대하게 받아들여서, 이름만 `.zip`인 진짜 깨진
  파일은 에러 대신 빈 압축파일로 열린다 — 최상위 "열기"를 포함해 이 앱
  전체가 이미 겪는 특성
...
- `P2` 손상된 압축파일 복구/부분 해제 시도 — ✅ 완료. 항목 하나가 손상돼
  읽지 못해도(깨진 압축 스트림 등) 그 항목만 건너뛰고 나머지는 계속
  해제한다 — 예전에는 항목 하나만 잘못돼도 전체 해제가 그 자리에서
  멈췄다. 완료되면 건너뛴 항목 개수와 그 목록(경로+사유)을 완료 스낵바에
  함께 보여주고, 복사 버튼으로 그대로 클립보드에 담을 수 있다
  (`ExtractFailure`/`ExtractResult`, `DartArchiveReader._extractEntries`,
  `ArchiveBrowserScreen._runExtraction` 참고). **진짜 바이트 단위 복구(깨진
  중앙 디렉터리 재구성 등)는 범위 밖** — "시도"의 의미는 손상된 부분을
  포기하고 나머지를 최대한 살리는 것이지, 손상 자체를 고치는 것은 아니다.
  비밀번호 문제나 사용자 취소는 이 부분 해제 대상이 아니다 — 그 둘은
  전체 작업을 멈추고 각자의 흐름(재시도, 취소)으로 그대로 이어진다.
...
- `P0` 파일/폴더 선택(드래그앤드롭 또는 파일 피커) → 새 압축파일 생성 — ✅
  완료. `file_selector`의 `openFiles()`는 macOS에서 폴더 선택
  (`canChooseDirectories`)을 지원하지 않아(플러그인 자체 제약), "새 압축
  만들기" 버튼이 "파일 선택.../폴더 선택..." 두 항목의 작은 메뉴가 됐다
  — 드래그앤드롭은 처음부터 폴더도 지원했다(파일시스템 경로일 뿐이라
  구분이 없음)
...
- `P2` 압축 시 파일 필터(특정 확장자 제외, 심볼릭 링크 처리 옵션) — ✅
  완료. `CompressionOptions`에 `excludedExtensions`(점 없이/소문자로
  정규화해 비교, `Makefile`처럼 확장자 없는 파일은 절대 안 걸림)와
  `followSymlinks`(기본값 false — 순환 링크 무한 루프나 예상 밖의 거대한
  대상을 조용히 끌어들이는 것을 막는 보수적 기본값) 두 필드를 추가했다.
  `DartArchiveWriter._collectEntries`가 폴더 순회 중 이 둘을 적용한다 —
  단, 소스로 직접 고른 파일 하나짜리 압축(gzip/bzip2/xz)에는 확장자
  필터를 적용하지 않는다(사용자가 명시적으로 고른 파일을 필터가 걸러
  아무것도 안 만드는 혼란을 피하기 위해). `CompressDialog`엔 두 옵션 다
  체크박스 뒤에 숨기지 않고 처음부터 노출했다 — 조건부로 나타나는
  TextField 뒤에 새 `showDialog`를 띄우면 이 Flutter SDK에서 시맨틱스
  단정문이 깨지는 기존에 발견된 프레임워크 문제(비밀번호/분할 압축
  테스트의 `skip: true` 참고)를 애초에 피하기 위해서다.
...
- `P2` OS 컨텍스트 메뉴("여기에 압축", "여기에 풀기") — ✅ **macOS만** 완료.
  Windows는 실제 Windows 머신에서 이어가고(이 macOS 개발 환경에서는
  툴체인이 없어 진행 불가 — 등록 방식·Dart 진입점·미결정 사항은
  ARCHITECTURE.md 11장의 "다음 버전 작업 메모" 참고, 원래 가정했던 COM
  DLL이 꼭 필요한지부터 재검토할 것), Linux(파일관리자별 플러그인)는 보류
  — 아래 "1차 범위 밖" 항목 참고. macOS는 Finder Sync
...
- `P2` 끌어내서 해제(압축 목록의 항목을 Finder/탐색기로 드래그) — ✅ 1단계
  ...
  프로미스") — ✅ **macOS 완료**(2026-09-24, 플러그인 v0.5.0). macOS에서는
  위 1단계 대신 끌기 시작할 때 아무것도 풀지 않고, 드롭되면 Finder가
  알려 준 위치(같은 이름이 있으면 Finder가 `이름 2`로 바꿔 줌)에 곧바로
  푼다(`DragOutStaging.extractItemTo` — 같은 볼륨의 숨김 임시 폴더에 푼
  뒤 그 항목만 이름 바꾸기로 옮김). 그래서 크기 제한·"아직 푸는 중"·임시
  폴더 정리가 없다. 여러 항목은 메모리를 아끼려고 하나씩 차례로 풀고,
  다 풀리면 완료/실패를 스낵바로 알린다. 비밀번호는 여전히 끌기 전에
  알고 있어야 한다. **Windows는 1단계 그대로** — 플러그인 0.6.0에
  Windows 프로미스가 생기면 붙인다(플러그인 저장소
  `doc/design/end-callback-and-file-promises.md`). Linux도 1단계.

## 2. 1차 범위 밖 (비목표)
- OS 셸 확장 — **macOS(NSServices)는 위에서 완료**. **Windows는 실제 Windows
  머신에서 이어간다**(2026-09-23 결정) — 등록·디버깅에 Windows 툴체인
  (Visual Studio, 레지스트리 등)이 있어야 해서 이 macOS 개발 환경에서는
  진행할 수 없다(구체적인 접근 방식은 ARCHITECTURE.md 11장 "다음 버전
  작업 메모" 참고 — 원래 가정했던 COM `IContextMenu`/`IExplorerCommand`
  DLL이 꼭 필요한지부터 재검토 대상). **Linux는 보류**(같은 날 결정) —
  데스크톱 환경별(GNOME/KDE 등 Nautilus/Dolphin/Thunar)로 구현이 전부
  다르고 공유 API가 없어, 지금 당장 계획이 없다.
...
| `P2` 생성+해제 | LZ4, Brotli (단일 파일) | 네이티브 백엔드 필요 — 다음 버전으로 연기(4장 상단 결정 참고) |
| `P1` **해제 전용** | RAR (RAR5 전체, RAR4는 대부분) | ✅ 완료 — `koni_rar`(순수 Dart). 아래 "RAR 정책" 참고 |
| `P2` **해제 전용** | ISO9660, CAB, ARJ, LHA/LZH, CPIO, AR, XAR, WARC | `libarchive` 커버리지 그대로 활용 — 다음 버전으로 연기(4장 상단 결정 참고) |
| `P2` **해제 전용** | DEB(`ar`+`tar`), RPM(`cpio` 부분) | 리눅스 패키지 내용물 열람용 — 다음 버전으로 연기(4장 상단 결정 참고) |
...
> **결정(다음 버전으로 연기)**: 이 장이 설명하는 libarchive/Rust 네이티브
> 백엔드 도입은 이번 버전 범위에서 제외하고 다음 버전으로 미룬다. 순수
> Dart(`archive` 패키지 + `koni_sevenz`/`koni_rar`)만으로 이번 세션에서
> P0/P1/P2 대부분(분할 압축, 손상 복구/부분 해제, macOS OS 통합, 압축 시
> 파일 필터 등)을 완료했고, 남은 것은 전부 이 네이티브 백엔드가 있어야만
> 가능한 항목들(zstd/lz4/brotli, iso9660/cab/arj/lha/cpio 등 해제 전용
> 레거시 포맷, 진짜 멀티볼륨 스펙 읽기, Windows/Linux OS 셸 확장)이다.
> 이 장의 나머지 내용(4.1~4.3)은 그 다음 버전을 위한 설계 초안으로 그대로
> 남겨둔다 — 지금 당장 착수할 항목이 아니라는 점만 기억할 것.
